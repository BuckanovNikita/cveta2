...
1. Интерактивный выбор классов (`instance_label`) через checkbox — изображения без аннотаций тоже можно включить
...
**Пометить кадры удалёнными** (`task mark-deleted`) — по ID кадра (`--frame`) и/или имени изображения (`--image`); оба флага можно повторять, нужен хотя бы один:

```bash
cveta2 task mark-deleted -p "Мой проект" -t 456 --frame 0 --frame 3
cveta2 task mark-deleted -p "Мой проект" -t 456 --image img001.jpg --image img002.jpg
```
...
По умолчанию изображения сохраняются прямо в указанную директорию (`/mnt/disk01/data/project_coco_8_dev/image.jpg`), без дополнительных подпапок. Настройка `cache.projects.<проект>.ignored_prefix` меняет это: из полного S3-ключа отбрасывается только указанный префикс, а остаток сохраняется как подпапки. Например, при `ignored_prefix: data` ключ `data/projA/2026-01/img.jpg` сохранится как `<директория>/projA/2026-01/img.jpg`.
