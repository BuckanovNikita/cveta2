...
Показывает задачи, завершённые **после** тех, что уже есть в выгруженном `dataset.csv`. Дата отсечки — максимальная `task_updated_date` среди строк со статусом `completed` (если таких нет — среди всех строк). Задачи, чей `task_id` уже присутствует в CSV, помечаются как `(обновлена)`.
...
- **Локально** — `~/.cache/cveta2/task_annotations/project_<id>/` (уважается `XDG_CACHE_HOME`). Корень можно сменить настройкой `cache.tasks_root` (глобально или по проектам).
