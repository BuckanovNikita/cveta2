...
- Собирает **все bbox-аннотации** проекта в плоский список — одна запись на каждый bounding box. Извлекаются только фигуры типа `rectangle`; `polygon`, `polyline`, `points` и другие типы CVAT пропускаются
