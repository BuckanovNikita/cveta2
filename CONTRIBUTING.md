...
- Охват задаётся в `[tool.mutmut].only_mutate`; счёт мутантов и score печатает
  ...
  не был красным. Вне гейта осознанно оставлены `services/fetch.py` (замерен,
  выжившие сосредоточены в кэш-цикле) и `_client/sdk_adapter.py` (граница SDK,
  сначала нужно покрытие) — см. CLAUDE.md.
