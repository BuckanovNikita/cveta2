...
```bash
./scripts/mutation_test.sh                       # весь текущий охват
...
исполняют код. Запускается в pre-commit на каждый коммит (~10 c) и падает,
если выжил хоть один мутант без объяснения.

- Охват задаётся в `[tool.mutmut].only_mutate` и расширяется по одному модулю:
  модуль добавляют в список тем же коммитом, который доводит его до нуля
  выживших. Сейчас в охвате `cveta2/dataset_partition.py`.
- Если хук упал — по умолчанию усильте тест. Мутанта, которого в принципе
  нельзя убить (мутация не меняет поведение), добавьте в
