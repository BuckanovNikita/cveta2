...
- Охват задаётся в `[tool.mutmut].only_mutate`. Сейчас туда входит 31 модуль
  (5120 мутантов, 98.4 % убито, 80 в allowlist, ноль необъяснённых) — то есть
  всё, что имеет смысл мутировать. Новый модуль в `cveta2/` добавляйте в
  `only_mutate` тем же коммитом, который доводит его до нуля выживших, чтобы
  гейт на `main` никогда не был красным.
- Если хук упал — по умолчанию усильте тест. Второй вариант: перестройте код
  так, чтобы мутанта не существовало (литералы, влияющие только на
  отображение, выносятся в константу уровня модуля — её mutmut не мутирует).
  Только если мутация в принципе не может изменить поведение, добавьте её в
  `[tool.cveta2.mutation.equivalent]` в `pyproject.toml` с обоснованием.
