...
cveta2/
  ...
  commands/     - CLI command implementations, split from cli.py
    ...
    labels.py   - run_labels(): list and interactively edit project labels (add/rename/delete); safety checks count annotation usage per label before deletion; uses CvatClient.update_project_labels() with PatchedProjectWriteRequest PATCH API
...
- `labels` — list and interactively edit project labels (add, rename, delete). Includes safety checks: before deletion, counts annotations per label across all tasks and warns the user how many annotations will be permanently destroyed; requires typing the label names to confirm. Arguments:
  ...
  - If `--list` is not given, opens interactive TUI for the selected project with actions: add label, rename label, delete label.
...
- **Label deletion safety**: CVAT permanently destroys all annotations (shapes, tracks) using a deleted label — there is no undo. `cveta2 labels` counts annotation usage across all tasks before allowing deletion. If annotations exist, the user must type the label names to confirm. Label renaming and adding are safe and do not affect existing annotations; renaming preserves all annotations via the label's `id`. The CVAT PATCH API (`PatchedProjectWriteRequest` with `PatchedLabelRequest`) applies only the listed changes — unlisted labels are left unchanged.
