...
- **Integration stack layout**: `integration_up.sh` always starts `cvat_ui` and `traefik` alongside the backend services. Single port (`CVAT_PORT`) serves both API and UI via traefik — `cvat_server` is not exposed on the host directly. All container names are prefixed with `${INTEGRATION_USER}` (sanitized `$USER`, default `default`) to avoid clashes between users on a shared machine. Override files: `docker-compose.override.yml` (backend services, MinIO, container_name overrides) and `docker-compose.ui.yml` (traefik port binding, cvat_ui/traefik container_name overrides).
