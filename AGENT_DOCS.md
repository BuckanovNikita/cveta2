...
```
cveta2/
  __init__.py   - public API re-exports: CvatClient, FetchContext, fetch_annotations, AnnotationRecord, BBoxAnnotation, DeletedImage, ImageWithoutAnnotations, ProjectAnnotations, TaskAnnotations, partition_annotations_df, PartitionResult, CSV_COLUMNS, Cveta2Error, ProjectNotFoundError, TaskNotFoundError, InteractiveModeRequiredError
  models.py     - Pydantic models: BBoxAnnotation, DeletedImage, ImageWithoutAnnotations, ProjectAnnotations, TaskAnnotations; AnnotationRecord discriminated union (BBoxAnnotation | ImageWithoutAnnotations, discriminator=instance_shape); CSV_COLUMNS tuple defining the canonical CSV column order; ProjectAnnotations.to_csv_rows() and TaskAnnotations.to_csv_rows() methods; TaskAnnotations.merge(list[TaskAnnotations]) → ProjectAnnotations
...
1. **Core architecture layers** (`layers`): enforces `cli → commands → client → _client` direction. Lower layers must not import from upper layers. Three `TYPE_CHECKING`-only imports from `commands/` into `_client.dtos` (for `RawLabel`/`RawTask` type hints) are allowed via `ignore_imports` since they create no runtime coupling.
