...
```
cveta2/
  __init__.py   - public API re-exports: CvatClient, FetchContext, fetch_annotations, AnnotationRecord, BBoxAnnotation, DeletedImage, ImageWithoutAnnotations, ProjectAnnotations, TaskAnnotations, ProjectInfo, TaskInfo, LabelInfo, LabelAttributeInfo, partition_annotations_df, PartitionResult, CSV_COLUMNS, Cveta2Error, ProjectNotFoundError, TaskNotFoundError, InteractiveModeRequiredError
  models.py     - Pydantic domain models: ProjectInfo (id, name), TaskInfo (id, name, status, subset, updated_date; format_display()), LabelAttributeInfo (id, name), LabelInfo (id, name, color, attributes; format_display()); annotation models: BBoxAnnotation, DeletedImage, ImageWithoutAnnotations, ProjectAnnotations, TaskAnnotations; AnnotationRecord discriminated union (BBoxAnnotation | ImageWithoutAnnotations, discriminator=instance_shape); CSV_COLUMNS tuple defining the canonical CSV column order; ProjectAnnotations.to_csv_rows() and TaskAnnotations.to_csv_rows() methods; TaskAnnotations.merge(list[TaskAnnotations]) → ProjectAnnotations
  ...
  image_downloader.py - CloudStorageInfo pydantic model; parse_cloud_storage() extracts bucket/prefix/endpoint from CVAT SDK cloud storage object; ImageDownloader class downloads images from S3 via boto3 into a flat target directory (no subdirs); uses only project_cloud_storage (no task storage); lists project prefix and downloads by name lookup; S3Syncer class lists all objects under an S3 prefix and downloads missing ones locally (never deletes, never uploads); _list_s3_objects() lists S3 objects with prefix stripping; _download_one_s3() shared download helper with tenacity retry; DownloadStats pydantic model (downloaded/cached/failed/total); _build_s3_key() handles prefix logic; tqdm progress bar
  projects_cache.py - YAML cache of project id/name list (load_projects_cache, save_projects_cache); path next to config (projects.yaml); ProjectInfo imported from models.py
  client.py     - CvatClient class (list_projects, resolve_project_id, get_project_labels, count_label_usage, update_project_labels, fetch_annotations, prepare_fetch, download_images, detect_project_cloud_storage, sync_project_images, create_upload_task, mark_frames_deleted, complete_task; usable as context manager for connection reuse) + fetch_annotations() DataFrame wrapper; FetchContext frozen dataclass (tasks, label_names, attr_names, host, project_name) returned by prepare_fetch(); _SdkClientFactory Protocol for typed client_factory parameter; all paths go through CvatApiPort for annotations; prepare_fetch() gets tasks/labels, filters, returns FetchContext; _fetch_one_task(api, task, ctx) static method fetches a single task and returns TaskAnnotations | None; fetch_annotations() is a convenience wrapper calling _prepare_fetch + loop + merge; 5xx handling: per-task ApiException with status 500–599 caught in _fetch_one_task, logged with task link + ignore-command hint, returns None; CVETA2_RAISE_ON_FAILURE=true causes immediate reraise; detect_project_cloud_storage() reads project's source_storage via projects.retrieve(project_id) (no task probing); download_images() and sync_project_images() accept optional project_cloud_storage to avoid duplicate detection; download_images() delegates to ImageDownloader; sync_project_images() delegates to S3Syncer; create_upload_task() creates a CVAT task backed by cloud storage images with segment_size controlling job size; mark_frames_deleted() marks frames as deleted via partial_update_data_meta; complete_task() sets all jobs to stage=acceptance, state=completed; _require_sdk() shared helper for methods needing the raw SDK; update_project_labels() supports add, rename, delete, and recolor (dict[int, str] mapping label_id → hex color)
  ...
  _client/      - internal implementation details split from client.py
    dtos.py     - frozen dataclass DTOs for internal CVAT API responses (RawFrame, RawShape, RawTrack, RawDataMeta, RawAnnotations, etc.); domain types (TaskInfo, LabelInfo, ProjectInfo) live in models.py
    ports.py    - CvatApiPort Protocol defining the API boundary; returns domain types (ProjectInfo, TaskInfo, LabelInfo) and internal DTOs (RawDataMeta, RawAnnotations); the single seam for mocking
    sdk_adapter.py - SdkCvatApiAdapter: CvatApiPort implementation wrapping an open cvat_sdk client; thin stateless converter (SDK objects in, domain models + DTOs out); tenacity retry with exponential backoff on all public methods; _extract_updated_date and _extract_creator_username use getattr for SDK version compat (intentional exception to style rule, documented inline). get_task_data_meta catches ApiTypeError for chunks_updated_date (CVAT can return null, SDK expects datetime) and falls back to raw GET + _data_meta_from_dict.
    context.py  - _TaskContext + extraction constants; frames typed as dict[int, RawFrame]; get_frame() and get_label_name() for extractors; from_raw(task, data_meta, label_names, attr_names) classmethod builds context from TaskInfo + DTOs
    mapping.py  - helper functions for label/attribute mapping; takes LabelInfo (from models) and RawAttribute (from dtos)
  ...
  commands/     - CLI command implementations, split from cli.py
    ...
    labels.py   - run_labels(): list and interactively edit project labels (add/rename/recolor/delete); safety checks count annotation usage per label before deletion; uses CvatClient.update_project_labels() with PatchedProjectWriteRequest PATCH API; recolor validates hex color format (#rrggbb) via _validate_hex_color(); same-color (case-insensitive) is treated as no-op
  ...
  cli.py        - slim argparse CLI entry point; CliApp class with parser definitions (fetch, fetch-task, setup, setup-cache, s3-sync, upload, merge, ignore, labels, doctor) and dispatch to commands/ modules; shared fetch args extracted into _add_common_fetch_args(); all command logic lives in commands/
...
**Projects** have their own `source_storage` (and `target_storage`) in the CVAT REST API (ProjectRead). We use only the **project's** `source_storage.cloud_storage_id` for image download and sync; we never use task-level `source_storage`. Detection is done via `sdk.projects.retrieve(project_id)` then reading `project.source_storage` (same dict/object handling as for tasks).
...
2. **Partition cached** — for each frame, check if `target_dir/image_name` already exists on disk. If so, increment `stats.cached` and skip.
3. **Project storage only** — we never use task cloud storage. When `project_cloud_storage` is provided (from `CvatClient.detect_project_cloud_storage(project_id)` which reads the project's own `source_storage` via `projects.retrieve(project_id)`), the downloader builds a single S3 client for that storage, lists all objects under the project prefix, builds a name→key map (by full name and basename), and downloads all pending images from that map; not found → `failed`. When `project_cloud_storage` is None, all pending images are counted as `failed`.
4. **S3 credentials** come from the standard boto3 chain (`~/.aws/credentials`, `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` env vars, IAM roles). No S3 credentials are stored in the cveta2 config. Each download is retried up to 3 times with exponential backoff via tenacity. Progress displayed via tqdm.
...
- `s3-sync` — syncs all images from S3 cloud storage to local cache for every project configured in `image_cache`. Lists all objects under each project's cloud storage prefix and downloads those missing locally. Never deletes from S3 or syncs in reverse. Arguments:
  ...
  - Flow: loads `image_cache` from config → for each project calls resolve_project_and_cloud_storage() (project's source_storage from projects.retrieve, no task probing) → if cloud storage found, lists S3 objects under prefix and downloads missing files to the configured cache directory. Continues to next project on errors (project not found, no cloud storage).

- `labels` — list and interactively edit project labels (add, rename, recolor, delete). Includes safety checks: before deletion, counts annotations per label across all tasks and warns the user how many annotations will be permanently destroyed; requires typing the label names to confirm. Arguments:
  - `--project` / `-p` — project ID or name; if omitted, interactive TUI selection.
  - `--list` — list labels with attributes and exit (non-interactive).
  - If `--list` is not given, opens interactive TUI for the selected project with actions: add label, rename label, change label color, delete label.
  - **Label color change**: Select a label, enter a new hex color (`#rrggbb`). Input is validated; entering the same color (case-insensitive) is a no-op. Color changes do not affect annotations.
  - **Label deletion safety**: CVAT permanently destroys all annotations (shapes, tracks) using a deleted label. The command fetches annotation counts per label and requires explicit confirmation (typing label names) when annotations exist.
...
- `upload` — creates a CVAT task from `dataset.csv`: reads the CSV, filters by class labels interactively, uploads images directly to S3 (skipping existing), and creates one task with multiple jobs (controlled by `segment_size`). Rows with `instance_shape="deleted"` are automatically detected, included in the task, and marked as deleted frames in CVAT. Arguments:
  - `--project` / `-p` — project ID or name; if omitted, interactive TUI selection.
  - `--dataset` / `-d` (required) — path to `dataset.csv` produced by `fetch`. May contain rows with `instance_shape="deleted"` — those images are uploaded and marked as deleted in CVAT automatically.
  ...
  - `--complete` — mark the task as completed after upload (sets all jobs to stage=acceptance, state=completed).
  - Flow: read dataset.csv → separate deleted rows (`instance_shape="deleted"`) from normal rows → read in_progress.csv (optional) → interactive `questionary.checkbox` for `instance_label` filtering (on normal rows only) → prompt task name → resolve image files on disk (search in `--image-dir` + project cache from `image_cache` config) → detect project cloud storage → upload missing images to S3 via `S3Uploader` (includes deleted images) → create CVAT task via `create_upload_task()` with `segment_size` from `UploadConfig` → upload annotations → mark deleted frames via `mark_frames_deleted()` → optionally complete task via `complete_task()` → log task URL.
...
- `README.md` — user-facing intro, CLI usage, config, examples (Russian). References `DATASET_FORMAT.md` for output file and data model details.
- `DATASET_FORMAT.md` — output CSV file descriptions (fetch / fetch-task output tables) and full field-level docs for BBoxAnnotation, ImageWithoutAnnotations, DeletedImage, DownloadStats, UploadStats (Russian).
- `AGENT_DOCS.md` — this file; project internals and implicit decisions (English).

## Data model notes

- `AnnotationRecord` — Pydantic discriminated union (`Annotated[BBoxAnnotation | ImageWithoutAnnotations, Discriminator("instance_shape")]`). `BBoxAnnotation` has `instance_shape="box"`, `ImageWithoutAnnotations` has `instance_shape="none"`. Both share `image_name`, `task_id`, `frame_id` and implement `to_csv_row()`. `DeletedImage` is separate from `AnnotationRecord` (not part of the union) and has `instance_shape="deleted"`. All three `instance_shape` values (`"box"`, `"none"`, `"deleted"`) are `Literal` types ensuring CSV column consistency.
...
- `TaskAnnotations` — per-task result: `task_id`, `task_name`, `annotations: list[AnnotationRecord]`, `deleted_images: list[DeletedImage]`. Has `to_csv_rows()` and static `merge(list[TaskAnnotations]) -> ProjectAnnotations` for combining per-task results.
- `DeletedImage` — record of a deleted frame: `image_name`, `image_width` (default 0), `image_height` (default 0), `instance_shape="deleted"` (discriminator value), `task_id`, `task_name`, `task_status`, `task_updated_date`, `frame_id`, `subset`. Written to `deleted.csv` with `instance_shape="deleted"` so the file shares the same column schema as `dataset.csv`.
...
`CvatClient` has a **single code path** for annotation logic split into `_prepare_fetch` and `_fetch_one_task` static methods (composed by `_fetch_annotations`), all working through `CvatApiPort` with domain models (`TaskInfo`, `LabelInfo`, `ProjectInfo` from `models.py`) and internal DTOs. Public API: `prepare_fetch()` returns a `FetchContext` (tasks + label maps); `fetch_annotations()` is a convenience wrapper. Internal callers (e.g. `_fetch_and_save_tasks` in commands) use `_api_or_adapter()` + `_fetch_one_task()` directly. All methods that need an API port (`list_projects`, `list_project_tasks`, `fetch_annotations`, `prepare_fetch`, `detect_project_cloud_storage`) use the **`_api_or_adapter()`** context manager: it yields the injected or persistent API when available, otherwise enters `_open_sdk_adapter()` and yields a temporary `SdkCvatApiAdapter`. So the "api vs open adapter" choice lives in one place; no duplicated `if api is not None` / `else: with _open_sdk_adapter()` branches. When `api` is injected (tests), the provided implementation is used directly. When `api` is `None` (production), `CvatClient` opens an SDK client via `client_factory` and wraps it in `SdkCvatApiAdapter` on the fly, so the same logic runs in both cases. All CVAT SDK interaction is isolated inside `SdkCvatApiAdapter`; no other module imports `cvat_sdk` (except `client.py` which imports `make_client` as the default factory). `SdkCvatApiAdapter` accepts an already-opened SDK client; `CvatClient` owns the client lifecycle. Domain models are frozen Pydantic models; internal DTOs are frozen dataclasses — both are easy to construct in test fixtures without any SDK dependency.
...
- **Loader**: `tests/fixtures/load_cvat_fixtures.py` — `load_cvat_fixtures(project_dir)` reads `project.json` and all `tasks/*.json`, returns `(ProjectInfo, list[TaskInfo], list[LabelInfo], dict[task_id, (RawDataMeta, RawAnnotations)])` using `cveta2.models` and `cveta2._client.dtos`. Used by tests and by `FakeCvatApi`.
...
- **Upload integration tests**: `tests/integration/test_upload.py` — end-to-end tests against a live, seeded CVAT + MinIO: S3Uploader (MinIO), create_upload_task, upload_task_annotations, full flow (create task → upload annotations → fetch and verify). Require `CVAT_INTEGRATION_HOST` and coco8-dev project with cloud storage (run `scripts/integration_up.sh` which seeds via `tests/integration/seed_cvat.py`). If `--port` is not passed, `integration_up.sh` uses a random free port; use the printed URL as `CVAT_INTEGRATION_HOST` when running pytest.
- **Integration stack layout**: `integration_up.sh` always starts `cvat_ui` and `traefik` alongside the backend services. Single port (`CVAT_PORT`) serves both API and UI via traefik — `cvat_server` is not exposed on the host directly. All container names are prefixed with `${INTEGRATION_USER}` (sanitized `$USER`, default `default`) to avoid clashes between users on a shared machine. Docker Compose project name is `${INTEGRATION_USER}-cvat` to isolate volumes/networks per user. Override file: `docker-compose.override.yml` (all service overrides, container_name prefixes, traefik port override via `!override`, MinIO with dynamic ports).

## Architecture enforcement (import-linter)

`import-linter` enforces architectural boundaries via contracts defined in `pyproject.toml`. Run manually: `uv run lint-imports`. Also runs as a pre-commit hook.

Three contracts:

1. **Core architecture layers** (`layers`): enforces `cli → commands → client → _client` direction. Lower layers must not import from upper layers. Domain types (`TaskInfo`, `LabelInfo`, `ProjectInfo`) live in `models.py` (foundation layer) and are imported by all layers without violating the architecture.
2. **Foundation modules isolation** (`forbidden`): `models` and `exceptions` must not import from `client`, `commands`, `cli`, or `_client`.
3. **Config layer isolation** (`forbidden`): `config` must not import from `client`, `commands`, `cli`, `_client`, or `models`. It may only depend on `exceptions`.

When adding new modules or cross-module imports, run `uv run lint-imports` to verify contracts still pass.
...
- **Label deletion safety**: CVAT permanently destroys all annotations (shapes, tracks) using a deleted label — there is no undo. `cveta2 labels` counts annotation usage across all tasks before allowing deletion. If annotations exist, the user must type the label names to confirm. Label renaming, recoloring, and adding are safe and do not affect existing annotations; renaming preserves all annotations via the label's `id`. The CVAT PATCH API (`PatchedProjectWriteRequest` with `PatchedLabelRequest`) applies only the listed changes — unlisted labels are left unchanged.
...
- **setup-cache default path sanitization**: When building the default path `cache_root / project_name`, `_cache_dir_for_project()` in `commands/setup.py` replaces path-unsafe characters in `project_name`: `/`, `\`, and `\x00` become `_`, so CVAT project names that contain path separators still produce a single directory name.
...
- **Unified fetch loop**: Both `fetch` and `fetch-task` commands use `_fetch_and_save_tasks()` — a shared per-task fetch loop that calls `client.prepare_fetch()` then iterates tasks via `client.fetch_one_task()`. Each task's annotations are saved as `output_dir/.tasks/task_{id}.csv` before merging via `TaskAnnotations.merge()`. The per-task CSVs enable future resumability (skip already-fetched tasks).
...
- **Cloud storage**: We never use task cloud storage. `CvatClient.detect_project_cloud_storage(project_id)` reads the **project's** `source_storage` via `sdk.projects.retrieve(project_id)` and `cloudstorages_api.retrieve(cloud_storage_id)`. Fetch and s3-sync use `resolve_project_and_cloud_storage()` to get (project_id, project_name, cs_info) once and pass `project_cloud_storage` into `download_images` / `sync_project_images`. The downloader only uses project storage: lists objects under the project prefix and downloads by name lookup; when `project_cloud_storage` is None, all pending images are counted as failed.
...
- **`s3-sync`** uses `resolve_project_and_cloud_storage()` which calls `CvatClient.detect_project_cloud_storage()` (project's `source_storage` from `projects.retrieve(project_id)`, no task probing). If the project has no source_storage, the project is skipped with a warning.
...
- **`upload` automatically handles deleted images**: Rows with `instance_shape="deleted"` in the dataset CSV are detected, their images are uploaded to S3 and included in the task, and then marked as deleted frames via `mark_frames_deleted()` (uses `tasks_api.partial_update_data_meta()`). Deleted rows are separated before label selection and do not participate in class filtering or annotation upload.
- **`upload --complete` marks the task as completed**: `complete_task()` retrieves all jobs for the task via `task_obj.get_jobs()` and updates each via `jobs_api.partial_update()` with `stage=acceptance` and `state=completed`. CVAT derives task status from its jobs, so the task becomes "completed" once all jobs are updated.
- **CVAT 5xx handling during fetch**: When fetching annotations per task, if CVAT returns HTTP 5xx for a task, the library catches `ApiException`, logs the error and task link (host/tasks/id or task_id/name), skips that task and continues with the rest. If env var `CVETA2_RAISE_ON_FAILURE=true` is set, the exception is re-raised immediately (no skip, whole fetch fails). When a task is skipped, the log includes a hint: `cveta2 ignore --project <project_name> --add <task_id>` so the user can add the broken task to the ignore list for the next run.
